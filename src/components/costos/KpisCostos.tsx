...
const KpisCostos: React.FC = () => {
  const { datos, recargarDatos, loading, error } = useCostos();
  useEffect(()=>{ recargarDatos(); }, [recargarDatos]);
  const k = datos?.kpis || {};
  const orden = ['valorTotal','costoTotal','ganancia','totalHoras','productividadPromedio'];
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
      {orden.map(key => (
        <div key={key} className="bg-white rounded-lg shadow-sm p-5 border border-gray-200">
