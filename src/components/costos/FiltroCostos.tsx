...
// ✅ FIX: Tipado fuerte para TipoVista
type TipoVista = 'diario' | 'semanal' | 'mensual' | 'rango';

const FiltroCostos: React.FC = () => {
  const { filtros, aplicarFiltros, recargarDatos, loading } = useCostos();
  const [inicio, setInicio] = useState(filtros.fechaInicio);
  const [fin, setFin] = useState(filtros.fechaFin);
  const [tipoVista, setTipoVista] = useState<TipoVista>(filtros.tipoVista);
  ...
  const calcularRango = useCallback((base: Date, tv: string): { ini:string; fin:string } => {
    ...
    if (tv === 'mensual') {
      // ✅ FIX CRÍTICO: Cálculo correcto del rango mensual usando UTC para consistencia con backend
      const year = d.getUTCFullYear(); // Usar getUTCFullYear para consistencia
      const month = d.getUTCMonth();   // Usar getUTCMonth para consistencia
      ...
      const hoy = new Date();
      const esMesActual = year === hoy.getUTCFullYear() && month === hoy.getUTCMonth();
    ...
    // ✅ FIX: Lógica correcta para el tipo 'rango'
    if (tv === 'rango') {
      const duracionMs = new Date(fin).getTime() - new Date(inicio).getTime();
      const finNuevo = new Date(base.getTime() + duracionMs);
      return { ini: format(base), fin: format(finNuevo) };
    }
  ...
  function onChangeTipoVista(tv: TipoVista){
    ...
    if (tv !== 'rango') {
      ...
      aplicarFiltros({ tipoVista: tv, fechaInicio: ini, fechaFin: f2 });
    } else {
      ...
      aplicarFiltros({ tipoVista: tv, fechaInicio: inicioRango, fechaFin: finRango });
    }
  }
  ...
  function moverPeriodo(delta:number){
    ...
    if (tipoVista === 'mensual') {
      ...
      // ✅ FIX: Navegación mensual usando UTC para consistencia
      ...
      base = new Date(Date.UTC(nuevoAño, mesNormalizado, 1));
    ...
    if (typeof window !== 'undefined') {
      Object.keys(localStorage).forEach(k => {
        if (k.startsWith('costos:')) localStorage.removeItem(k);
      });
    }
  ...
  // ✅ FIX: Control de deshabilitado simplificado y más confiable
  const disableForward = (() => {
    ...
    if (tipoVista === 'mensual') {
      ...
      // Obtenemos el mes del inicio actual
      const year = parseInt(inicio.slice(0, 4));
      const month = parseInt(inicio.slice(5, 7)) - 1; // Convertir a base 0
      const mesActualFiltro = year * 12 + month;
      
      // ✅ FIX: Deshabilitar solo si estamos en el mes actual (permitir navegación hacia atrás)
    ...
    // Para diario y semanal, comparamos fechas
      ...
      if (tipoVista === 'diario') {
        // Para diario, deshabilitar si el siguiente día es hoy o futuro
        const siguienteDia = new Date(inicioDate);
        siguienteDia.setDate(siguienteDia.getDate() + 1);
        return siguienteDia >= hoy;
      } else if (tipoVista === 'semanal') {
        // Para semanal, deshabilitar si la siguiente semana incluye hoy o futuro
  ...
  return (
    <div className="bg-gray-50 rounded-lg shadow-md p-4 border border-gray-300 flex flex-col md:flex-row md:items-end gap-4">
      <div className="flex flex-col gap-1 min-w-[140px]">
        <label className="text-xs text-gray-700 font-medium">Vista (Costos)</label>
        <select value={tipoVista} onChange={e=>onChangeTipoVista(e.target.value as TipoVista)} className="border rounded-md px-2 py-1 text-sm bg-white focus:ring-2 focus:ring-blue-500">
      ...
      <div className="flex flex-col gap-1">
        ...
        <input disabled={tipoVista!=='rango'} type="date" value={inicio} onChange={e=>setInicio(e.target.value)} className="border rounded-md px-2 py-1 text-sm bg-white disabled:bg-gray-100" />
      </div>
      <div className="flex flex-col gap-1">
        <label className="text-xs text-gray-700 font-medium">Hasta</label>
        <input disabled={tipoVista!=='rango'} type="date" value={fin} onChange={e=>setFin(e.target.value)} className="border rounded-md px-2 py-1 text-sm bg-white disabled:bg-gray-100" />
      </div>
      <div className="flex gap-2 items-center mt-2 md:mt-0">
        <button onClick={()=>moverPeriodo(-1)} className="px-2 py-1.5 text-sm rounded-md border bg-white hover:bg-gray-100">◀</button>
        <button onClick={()=>!disableForward && moverPeriodo(1)} disabled={disableForward} className={`px-2 py-1.5 text-sm rounded-md border bg-white ${disableForward? 'opacity-40 cursor-not-allowed' : 'hover:bg-gray-100'}`}>▶</button>
      </div>
      <div className="flex flex-col gap-1 min-w-[140px]">
        <label className="text-xs text-gray-700 font-medium">Periodo</label>
        <div className="text-xs px-2 py-1 rounded-md border bg-white font-mono" title={periodoLabel}>{periodoLabel}</div>
      </div>
      {tipoVista==='rango' && <button onClick={aplicarManual} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm shadow">Aplicar</button>}
      <button 
        onClick={async () => {
          // Limpiar todo el caché relacionado con costos
          if (typeof window !== 'undefined') {
            Object.keys(localStorage).forEach(k => {
              if (k.startsWith('costos:')) localStorage.removeItem(k);
            });
